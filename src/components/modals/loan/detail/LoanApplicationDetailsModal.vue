  ...
  <teleport to="body">
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
      <div
        ...
        <div v-if="!isEditingInModal && !isDraft"
...
import { formatPrice, formatCurrencyInput, getStatusBadgeClass, getStatusText } from '@/utils/formatters'
...
const getErrorMessage = (error: unknown, defaultMessage: string) => {
  const errObj = error as { response?: { data?: { message?: string; error?: string; errors?: string[] } }; message?: string };
...
const modalLoanForm = reactive({
  ...
  province_id: '', district_id: '', date_of_birth: '',
...
const calculatedAge = computed(() => {
  ...
  const birthDate = new Date(modalLoanForm.date_of_birth);
...
interface UploadedFile { id?: number; file: File | Blob | null; preview: string; isPdf: boolean; name: string; }
    ...
    loanApplicationStore.currentDocuments.forEach(serverDoc => {
      ...
      if (fileUrl) {
        const newFileObj: UploadedFile = {
          id: serverDoc.id, file: null, preview: fileUrl, isPdf: serverDoc.file_url?.toLowerCase().endsWith('.pdf') || false,
...
watch(() => props.show, async (newVal) => {
  if (newVal && props.loanId) {
    try {
      ...
      try { selectedContract.value = await loanContractStore.fetchContract(props.loanId) } catch { }
      ...
      if (props.openInEdit && (permissionStore.hasPermission('loan_edit') || permissionStore.hasPermission('loan_create'))) {
        ...
        activeTab.value = 'details'; isEditingInModal.value = false
      }
    } catch { closeModal() }
  } else { selectedLoan.value = null; selectedContract.value = null; }
})
...
const canManageLocation = computed(() => permissionStore.hasPermission('loan_edit') || permissionStore.hasPermission('loan_create'));
...
const loadVariantsForEdit = async (productId: number, variantId?: number) => {
  ...
  try {
    ...
    if (variantId) { modalSelectedVariant.value = modalProductVariants.value.find(v => v.id === variantId) || null; }
    else { modalSelectedVariant.value = null; }
  } catch { modalProductVariants.value = []; }
  finally { isModalLoadingVariants.value = false; }
}
...
const startEditInModal = async () => {
  ...
  if (loanData.customer?.date_of_birth) {
    const splitDate = new Date(loanData.customer.date_of_birth).toISOString().split('T');
    modalLoanForm.date_of_birth = splitDate[0] || '';
  } else { modalLoanForm.date_of_birth = ''; }
...
const saveLoanFromModal = async () => {
  if (!modalLoanForm.date_of_birth) {
    customAlert.error('ກະລຸນາກວດສອບ', 'ກະລຸນາເລືອກ ວັນເດືອນປີເກີດ!'); return;
  }
  if (calculatedAge.value < 18) {
    customAlert.error('ບໍ່ຜ່ານເງື່ອນໄຂ', `ຜູ້ຂໍສິນເຊື່ອຕ້ອງອາຍຸ 18 ປີຂຶ້ນໄປ (ປັດຈຸບັນອາຍຸ ${calculatedAge.value} ປີ)`); return;
  }
  if (modalProductVariants.value.length > 0 && !modalSelectedVariant.value) {
    customAlert.error('ກະລຸນາເລືອກ ສີ/ຂະໜາດ ຂອງສິນຄ້າກ່ອນບັນທຶກ'); return;
  }
  ...
  try {
    ...
    const updateData = {
      ...
      phone: modalLoanForm.customer_phone, identity_number: modalLoanForm.customer_id_card.trim(),
      account_number: modalLoanForm.account_number.trim() || null,
      date_of_birth: modalLoanForm.date_of_birth, age: calculatedAge.value,
      address: modalLoanForm.customer_address, province_id: modalLoanForm.province_id, district_id: modalLoanForm.district_id,
      occupation: modalLoanForm.occupation, income_per_month: modalLoanForm.monthly_income,
    ...
    const errorMsg = getErrorMessage(error, 'ເກີດຂໍ້ຜິດພາດໃນການບັນທຶກ');
...
const saveDocumentsOnly = async () => {
  ...
  try {
    ...
    if (filesToUpload.length > 0) {
      ...
      await loanApplicationStore.uploadMultipleDocuments(selectedLoan.value.customer_id, selectedLoan.value.id, filesToUpload, docTypesArray);
      if (typeof populateDocumentsFromStore === 'function') { populateDocumentsFromStore(); }
      customAlert.success('ອັບໂຫຼດເອກະສານສຳເລັດ!');
    } else { customAlert.info('ບໍ່ມີເອກະສານໃໝ່ໃຫ້ອັບໂຫຼດ'); }
    isEditingInModal.value = false;
  } catch (error: unknown) {
    const errorMsg = getErrorMessage(error, 'ເກີດຂໍ້ຜິດພາດການບັນທຶກເອກະສານ');
    customAlert.error('ເກີດຂໍ້ຜິດພາດການບັນທຶກເອກະສານ', errorMsg);
  } finally { isSaving.value = false; isUploadingDocuments.value = false; }
}

// 🟢 ຟັງຊັນບັນທຶກສັນຍາທີ່ຖືກຕ້ອງ
const handleSaveContract = async (_customerId: number, formData: any) => {
  if (!selectedLoan.value) return;
  isSaving.value = true;
  try {
    ...
    const cData = formData?.customer || {}; const wData = formData?.work || {};
    const pData = formData?.product || {}; const sData = formData?.shop || {};
    const gData = hasGuarantorOrRef ? (formData?.guarantor || {}) : {}; const gwData = hasGuarantorOrRef ? (formData?.guarantorWork || {}) : {};
    ...
    const updateAppPayload = {
      ...
      monthly_pay: Number(pData.monthlyPayment) || 0, fee: Number(pData.fee) || 0, payment_day: Number(pData.paymentDay) || 1,
      interest_type: pData.interestType || 'flat_rate', interest_rate_type: pData.interestRateType || 'monthly',
      variant_id: Number(pData.variantId) || selectedLoan.value?.variant_id || null,
      first_name: firstName, last_name: lastName, phone: cData.phone || '', identity_number: cData.idCard || '',
      gender: cData.gender || '', marital_status: cData.maritalStatus || '',
      date_of_birth: cData.dob || '', age: Number(cData.age) || 0, occupation: cData.occupation || '',
    ...
    const errorMsg = getErrorMessage(error, 'ເກີດຂໍ້ຜິດພາດໃນການບັນທຶກສັນຍາ ກະລຸນາລອງໃໝ່');
    customAlert.error('ເກີດຂໍ້ຜິດພາດ', errorMsg);
  } finally { isSaving.value = false; }
}
...
const clearModalProductSelection = () => {
  ...
  activeModalCampaign.value = false;
  handleModalCalculationChange();
}
...
const handleDocumentUpload = async (typeId: string, event: Event) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (currentFile.size > 8 * 1024 * 1024) { customAlert.error('ໄຟລ໌ໃຫຍ່ເກີນໄປ', `ໄຟລ໌ ${currentFile.name} ໃຫຍ່ເກີນໄປ (ເກີນ 8MB)`); continue; }
    let finalFile: File = currentFile;
    if (finalFile.type.startsWith('image/')) {
      try {
        const compressedBlob = await imageCompression(currentFile, { maxSizeMB: 3, maxWidthOrHeight: 1920, useWebWorker: true });
        finalFile = new File([compressedBlob], currentFile.name, { type: compressedBlob.type, lastModified: Date.now() });
      } catch { if (finalFile.size > 3 * 1024 * 1024) continue; }
    }
...
const removeDocument = (typeId: string, fileIndex: number) => {
  ...
  if (category && category.files[fileIndex]) {
    if (category.files[fileIndex].id) { if (confirm('ທ່ານຕ້ອງການລຶບເອກະສານນີ້ອອກຈາກລະບົບແທ້ບໍ່?')) category.files.splice(fileIndex, 1); }
    else { category.files.splice(fileIndex, 1); }
  }
}

const loadCustomerLocations = async (customerId: number) => { const { getCustomerLocations } = await import('@/api/customer'); customerLocations.value = (await getCustomerLocations(customerId)).data || [] }
...
const handleUpdateLocation = async (id: number, data: any) => { const { updateCustomerLocation } = await import('@/api/customer'); await updateCustomerLocation(id, data); if (selectedLoan.value?.customer_id) loadCustomerLocations(selectedLoan.value.customer_id) }
const handleDeleteLocation = async (id: number) => { const { deleteCustomerLocation } = await import('@/api/customer'); await deleteCustomerLocation(id); if (selectedLoan.value?.customer_id) loadCustomerLocations(selectedLoan.value.customer_id) }
const handleSetPrimary = async (id: number) => { const { updateCustomerLocation } = await import('@/api/customer'); await updateCustomerLocation(id, { is_primary: 1 }); if (selectedLoan.value?.customer_id) loadCustomerLocations(selectedLoan.value.customer_id) }
</script>
