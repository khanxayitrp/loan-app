      ...
      <div
        ...
        <div class="flex w-full overflow-x-auto no-scrollbar border-b border-gray-200 dark:border-gray-700 mb-6">
          <button @click="activeTab = 'details'"
            ...
            :class="activeTab === 'details' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-primary hover:bg-gray-50'">
          ...
          <button @click="activeTab = 'loanContract'"
            class="flex-1 flex flex-col items-center justify-center gap-1.5 py-3 px-2 min-w-[80px] border-b-2 transition-all whitespace-nowrap"
            :class="activeTab === 'loanContract' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-primary hover:bg-gray-50'">
...
const loadVariantsForEdit = async (productId: number, variantId?: number) => {
  ...
  } catch (error) { modalProductVariants.value = []; } 
...
// 🟢 ຟັງຊັນບັນທຶກສັນຍາທີ່ຖືກຕ້ອງ (ACID Compliance Payload)
