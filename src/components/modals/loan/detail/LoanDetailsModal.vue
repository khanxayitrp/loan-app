      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-6 w-full max-w-4xl mx-auto max-h-[90vh] overflow-y-auto">
              ...
              <div class="bg-slate-50 p-4 rounded-xl border border-slate-200">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div class="form-control">
                    ...
                    <input v-model="modalLoanForm.customer_name" type="text" class="input input-sm input-bordered w-full bg-white"
                  ...
                  <div class="form-control">
                    ...
                    <input v-model="modalLoanForm.customer_phone" type="tel" class="input input-sm input-bordered w-full bg-white"
                  ...
                  <div class="form-control">
                    ...
                    <input v-model.number="modalLoanForm.age" type="number" class="input input-sm input-bordered w-full bg-white" min="0" />
                  </div>
                  <div class="form-control">
                    <label class="label"><span class="label-text font-medium">ອາຊີບ *</span></label>
                    <input v-model="modalLoanForm.occupation" type="text" class="input input-sm input-bordered w-full bg-white"
                  ...
                  <div class="form-control mt-4 md:col-span-2">
                    ...
                    <div class="address-grid-custom mt-1">
                      <div class="input-sub">
                        ...
                        <input v-model="modalLoanForm.customer_address" type="text" class="input input-sm input-bordered w-full bg-white" placeholder="ບ້ານ" required />
                      </div>
                      <div class="input-sub">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="form-control">
                  ...
                  <input type="text" :value="formatCurrencyInput(modalLoanForm.total_amount)"
                    ...
                    class="input input-sm input-bordered w-full font-bold text-primary bg-white" required placeholder="ປ້ອນລາຄາສິນຄ້າ" />
                </div>
                ...
                <div class="form-control">
                  ...
                  <input type="text" :value="formatCurrencyInput(modalLoanForm.down_payment)"
                    @input="handleModalDownPaymentInput" class="input input-sm input-bordered w-full bg-white" placeholder="ປ້ອນເງິນດາວ" />
                </div>
                ...
                <div class="form-control">
                  ...
                  <input v-model.number="modalLoanForm.interest_rate" type="number"
                    class="input input-sm input-bordered w-full bg-gray-100 cursor-not-allowed"
                ...
                <div class="form-control">
                  ...
                  <input :value="formatPrice(modalLoanForm.monthly_payment)" type="text"
                    class="input input-sm input-bordered w-full bg-blue-50 text-blue-700 font-bold cursor-not-allowed" readonly />
                </div>
              </div>
            </div>
          </div>
        ...
        <div v-else-if="activeTab === 'documents'" class="space-y-6">
          ...
          <div v-if="!isEditingInModal" class="space-y-6">
            <div v-if="!loanApplicationStore.currentDocuments || loanApplicationStore.currentDocuments.length === 0" class="text-center py-12 text-gray-500">
            ...
            <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div v-for="doc in loanApplicationStore.currentDocuments" :key="doc.id" class="border rounded-lg p-4 flex flex-col gap-3 bg-white">
                <div class="flex justify-between items-start">
                  ...
                  <a :href="getFullImageUrl(doc.file_url) || '#'" target="_blank" download class="btn btn-xs btn-ghost text-primary hover:bg-primary/10">
          ...
          <div v-else class="space-y-6">
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div v-for="(doc, index) in loanDocuments" :key="'req-'+index" class="border-2 border-dashed rounded-lg p-4 bg-gray-50">
            ...
            <div class="mt-8">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div v-for="(doc, index) in optionalDocuments" :key="'opt-'+index" class="border-2 border-dashed rounded-lg p-4 bg-gray-50">
                  <div class="flex items-center justify-between mb-3">
                    <div>
                      <h5 class="font-medium">{{ doc.name }}</h5>
                      <p class="text-xs text-gray-500 mt-1">{{ doc.description }}</p>
        ...
        <div v-else-if="activeTab === 'map'" class="space-y-6">
          <CustomerLocationMap v-if="selectedLoan?.customer_id" :customer-id="selectedLoan.customer_id" :locations="customerLocations"
        ...
        <div class="flex justify-end gap-3 mt-6 border-t pt-6">
          ...
          <button v-if="!isEditingInModal && selectedLoan?.status !== 'closed' && selectedLoan?.status !== 'disbursed' && (activeTab === 'details' || activeTab === 'documents')"
            class="btn btn-primary" @click="startEditInModal">
            <span class="icon-[tabler--edit] size-4 mr-1"></span> ແກ້ໄຂ
          </button>
          
          <button v-else-if="isEditingInModal && activeTab === 'details'"
            class="btn btn-success text-white" @click="saveLoanFromModal" :disabled="isSaving">
            <span v-if="isSaving" class="loading loading-spinner loading-xs"></span>
            <span v-else class="icon-[tabler--device-floppy] size-4 mr-1"></span>
            <span v-if="!isSaving">ບັນທຶກການປ່ຽນແປງ</span>
          </button>

          <button v-else-if="isEditingInModal && activeTab === 'documents'"
            class="btn btn-success text-white" @click="saveDocumentsOnly" :disabled="isSaving">
            <span v-if="isSaving" class="loading loading-spinner loading-xs"></span>
            <span v-else class="icon-[tabler--device-floppy] size-4 mr-1"></span>
...
const modalFormErrors = reactive({
  ...
  province_id: '', district_id: '', total_amount: '', down_payment: '', interest_rate: '', loan_period: '', monthly_income: ''
})
