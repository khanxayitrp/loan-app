<template>
            ...
            <div v-if="selectedDraft?.customer" class="border-t pt-4 mt-4">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p class="font-medium">{{ selectedDraft.customer?.identity_number || '-' }}</p>
                </div>
                <div>
                  <label class="text-sm font-medium text-gray-500">ວັນເດືອນປີເກີດ (ອາຍຸ)</label>
                  <p>
                    {{ selectedDraft.customer?.date_of_birth ? new Date(selectedDraft.customer.date_of_birth).toLocaleDateString('lo-LA') : '-' }} 
                    <span class="text-gray-500">({{ selectedDraft.customer?.age || 0 }} ປີ)</span>
                  </p>
                </div>
                <div>
  ...
  <input v-model="modalDraftForm.customer_id_card" type="text" class="input input-sm input-bordered w-full bg-white" />
                  ...
                  <div class="form-control">
                    ...
                    <input v-model="modalDraftForm.date_of_birth" type="date"
                      class="input input-sm input-bordered w-full bg-white" required />
                  </div>
                  <div class="form-control">
                    <label class="label"><span class="label-text font-medium">ອາຍຸ (ປີ)</span></label>
                    <input :value="calculatedAge" type="number"
                      class="input input-sm input-bordered w-full bg-gray-100 cursor-not-allowed font-medium text-primary" readonly />
                  </div>
              ...
              <div v-if="selectedModalProduct"
                ...
                <label class="label pt-0">
                  <span class="label-text font-medium text-gray-800 dark:text-white">ເລືອກ ສີ / ຂະໜາດ <span
                      v-if="modalProductVariants.length > 0" class="text-error">*</span></span>
                </label>
            ...
            <div class="border-t pt-6">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="form-control">
                  <label class="label">
                    <span class="label-text font-medium">ອັດຕາດອກເບ້ຍ (ອັດຕະໂນມັດ) <span class="text-error">*</span> <span
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-4">
                <div v-for="(f, fileIndex) in cat.files" :key="fileIndex"
                  ...
                  <button type="button" @click.stop="removeDocument(cat.id, fileIndex)"
                    class="absolute top-2 right-2 bg-red-500 text-white rounded-full p-1 z-20 shadow-md hover:scale-110 transition-transform">
                    <span class="icon-[tabler--x] size-4"></span>
                  </button>
                  ...
                  <!-- 🟢 NEW BADGE -->
                  <div class="absolute bottom-1.5 left-1.5 pointer-events-none z-10">
                    <span v-if="f.file" class="text-[10px] bg-green-500 text-white px-2 py-0.5 rounded shadow font-bold">ໃໝ່</span>
                  </div>
                </div>
        ...
        <div v-else-if="activeTab === 'map'" class="space-y-4">
          <div v-if="customerLocations.length === 0" class="alert alert-warning shadow-sm py-2">
            ...
            <span class="text-sm">ລູກຄ້າຄົນນີ້ຍັງບໍ່ມີຂໍ້ມູນທີ່ຕັ້ງເທື່ອ.</span>
          </div>
...
const modalDraftForm = reactive({
  customer_name: '', customer_phone: '', customer_id_card: '', account_number: '', customer_address: '', occupation: '',
  date_of_birth: '', age: 0, province_id: '', district_id: '',
...
const calculatedAge = computed(() => {
  if (!modalDraftForm.date_of_birth) return 0;
  const today = new Date();
  const birthDate = new Date(modalDraftForm.date_of_birth);
...
const startEditInModal = async () => {
  ...
  modalDraftForm.account_number = draftData.customer?.account_number || ''
  modalDraftForm.customer_address = getDraftAddress(draftData)
  
  if (draftData.customer?.date_of_birth) {
    const splitDate = new Date(draftData.customer.date_of_birth).toISOString().split('T');
    modalDraftForm.date_of_birth = splitDate[0] || '';
  } else { 
    modalDraftForm.date_of_birth = ''; 
  }
...
const saveDraftFromModal = async () => {
  ...
  if (modalProductVariants.value.length > 0 && !modalSelectedVariant.value) {
    customAlert.error('ກະລຸນາກວດສອບ', 'ກະລຸນາເລືອກ ສີ/ຂະໜາດ ຂອງສິນຄ້າກ່ອນບັນທຶກ'); return;
  }
  ...
  try {
    ...
    const updateData = {
      ...
      phone: modalDraftForm.customer_phone, 
      identity_number: modalDraftForm.customer_id_card.trim(),
      account_number: modalDraftForm.account_number.trim() || null, 
      date_of_birth: modalDraftForm.date_of_birth, age: calculatedAge.value,
      address: modalDraftForm.customer_address, province_id: modalDraftForm.province_id, district_id: modalDraftForm.district_id,
      occupation: modalDraftForm.occupation, income_per_month: modalDraftForm.income_per_month,
    ...
    isEditingInModal.value = false
    customAlert.success('ບັນທຶກການແກ້ໄຂສຳເລັດ!')
    emit('refresh')
  } catch (error: any) {
    const errorMsg = getErrorMessage(error, 'ເກີດຂໍ້ຜິດພາດໃນການບັນທຶກ ກະລຸນາລອງໃໝ່');
...
const saveDocumentsOnly = async () => {
  ...
  try {
    ...
    if (filesToUpload.length > 0) {
      await loanApplicationStore.uploadMultipleDocuments(selectedDraft.value.customer_id, selectedDraft.value.id, filesToUpload, docTypesArray);
      await loanApplicationStore.fetchDocuments(selectedDraft.value.customer_id);
      populateDocumentsFromStore();
...
// ----------------- ການຈັດການເອກະສານແນບ ແລະ ແຜນທີ່ -----------------
const handleDocumentUpload = async (typeId: string, event: Event) => {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (finalFile.type.startsWith('image/')) {
      ...
      } catch (error) { if (finalFile.size > 3 * 1024 * 1024) continue; }
    }
...
const removeDocument = (typeId: string, fileIndex: number) => {
  ...
  if (category && category.files[fileIndex]) {
    if (category.files[fileIndex].id) {
      if (confirm('ທ່ານຕ້ອງການລຶບເອກະສານນີ້ອອກຈາກລະບົບແທ້ບໍ່?')) category.files.splice(fileIndex, 1);
    } else { category.files.splice(fileIndex, 1); }
  }
}

// ----------------- ຟັງຊັນສັນຍາ (Loan Contract) -----------------
const handleSaveContract = async (customerId: number, formData: any) => {
  ...
  try {
    ...
    const cData = formData?.customer || {}; const wData = formData?.work || {}; const pData = formData?.product || {}; const sData = formData?.shop || {};
    ...
    customAlert.success('ບັນທຶກຂໍ້ມູນສັນຍາສຳເລັດ!');
    isEditingInModal.value = false;
