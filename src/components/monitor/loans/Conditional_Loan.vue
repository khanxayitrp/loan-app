    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400">
          ລາຍການສິນເຊື່ອທີ່ອະນຸມັດ ແຕ່ມີຄະແນນປະເມີນລະຫວ່າງ 65 - 79
    ...
    <div
      ...
      <div>
        <label class="label pb-1">
          <span class="label-text text-sm font-medium text-gray-700 dark:text-gray-300">ຄົ້ນຫາ (ຊື່, ເບີໂທ, ເລກສິນເຊື່ອ,
            ເລກສັນຍາ)</span>
        </label>
        <div class="relative">
...
// ===============================================
// 🌟 3. ລະບົບຄົ້ນຫາໜ້າເວັບ (Local Filtering 100%)
// ===============================================
...
const debounceSearch = () => {
  ...
  debounceTimer = setTimeout(() => {
    ...
    resetPage(); // 🌟 ກັບໄປໜ້າທຳອິດເມື່ອພິມຄົ້ນຫາ
  }, 300);
};
...
const applyDateFilter = () => {
  resetPage(); // 🌟 ກັບໄປໜ້າທຳອິດເມື່ອປ່ຽນວັນທີ
};

// 🌟 4. ຟັງຊັນກັ່ນຕອງຂໍ້ມູນແບບ Local Filter
const filteredLoans = computed(() => {
  let filtered = loanApplications.value;

  // ค้นหาด้วยชื่อ, เบอร์โทร, เลขสินเชื่อ, หรือ เลขสัญญา
  ...
  // ค้นหาด้วยวันที่ (แก้ไขแล้ว: || '')
  if (dateFrom.value || dateTo.value) {
    filtered = filtered.filter(loan => {
      const targetDate = loan.approved_at || loan.updatedAt || loan.createdAt;
      if (!targetDate) return false;
      const loanDate = new Date(targetDate).toISOString().split('T')[0] || '';
  ...
  // 🌟 รักษาเงื่อนไขคะแนน: เปลี่ยน < 80 เป็น <= 79
  filtered = filtered.filter(loan => {
...
// 🌟 5. Pagination Computed Properties
...
const formatDate = (dateString: string | undefined): string => {
  ...
  // ດຶງຄ່າ ວັນ, ເດືອນ, ປີ ອອກມາແລ້ວຈັດຮູບແບບເອງໃຫ້ເປັນ DD/MM/YYYY
...
const exportToCSV = () => {
  ...
  const csvData = filteredLoans.value.map(loan => {
    ...
    return {
      ...
      'ວັນທີອະນຸມັດ': formatDate(loan.approved_at || loan.updatedAt || loan.createdAt || '')
    };
  });
...
// 🌟 6. ດຶງຂໍ້ມູນມາກ້ອນໃຫຍ່ (limit: 1000)
const fetchLoans = async () => {
  try {
    await loanApplicationStore.fetchLoanApplications({
      status: ['approved', 'disbursed', 'completed', 'closed'] as any,
      is_confirmed: 1,
      limit: 100,
      cursor: undefined,
      minScore: 65,
      maxScore: 79, // 🌟 ປ່ຽນເປັນ 79 ໃຫ້ກົງກັບ UI
    } as any);
  } catch (error: any) {
